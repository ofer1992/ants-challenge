...
class Ants():
    ...
    def setup(self, data):
        ...
        self.map_mat = np.full((self.rows, self.cols), M_LAND)
    ...
    def ant_observation(self, loc): # TODO: verify that it is working correctly
        ...
        obs = self.map_mat[np.ix_(rows, cols)]
        return obs
    ...
    def action_func(state):
        ...
        actions = [NOOP]
        for a in AIM:
            adj = AIM[a][0]+side+1, AIM[a][1]+side+1
            if state[adj] == M_LAND:
                actions += [a]
        return actions
