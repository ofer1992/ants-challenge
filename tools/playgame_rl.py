...
try:
    from engine_rl import run_game
except ImportError:
...
# this split of options is not needed, but left for documentation
    ...
    for round in range(opts.rounds):
        ...
        bots = [(RL_WD, RL_CMD)] + bots # We automatically add our RL bot
        bot_count = len(bots)
        ...
        if game.num_players != len(bots):
            if game.num_players > len(bots) and opts.fill:
                ...
                break
        bot_count = len(bots) # TODO: Might need tampering here later.
        ...
        result = run_game(game, bots, engine_options, rl_sandbox)
